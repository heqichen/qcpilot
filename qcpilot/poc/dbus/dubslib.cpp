...
/* Created and copyrighted by Zachary J. Fields. Offered as open source under
 * the MIT License (MIT). */
  ...
  if (nullptr == (dbus_conn = ::dbus_bus_get(DBUS_BUS_SYSTEM, &dbus_error))) {
    ...
    // Invoke remote procedure call, block for response
             ...
             (dbus_reply = ::dbus_connection_send_with_reply_and_block(
                  dbus_conn, dbus_msg, DBUS_TIMEOUT_USE_DEFAULT,
                  &dbus_error))) {
    ...
    // Parse response
  } else if (!::dbus_message_get_args(dbus_reply, &dbus_error, DBUS_TYPE_STRING,
