...
class Controls:
    def __init__(self) -> None:
        ...
        self.CP = messaging.log_from_bytes(
            self.params.get("CarParams", block=True), car.CarParams)
    ...
    def state_control(self):
        ...
        CC.enabled = self.sm['qcPilotCufuState'].isEngaged
        latActive = self.sm['qcPilotCufuState'].isLateralActive

        # Check which actuators can be enabled
        standstill = abs(CS.vEgo) <= max(self.CP.minSteerSpeed,
                                         MIN_LATERAL_CONTROL_SPEED) or CS.standstill
        CC.latActive = latActive and not CS.steerFaultTemporary and not CS.steerFaultPermanent and not standstill
        CC.longActive = CC.enabled and not any(
            e.overrideLongitudinal for e in self.sm['onroadEvents']) and self.CP.openpilotLongitudinalControl
        ...
        self.desired_curvature, curvature_limited = clip_curvature(
            CS.vEgo, self.desired_curvature, model_v2.action.desiredCurvature, lp.roll)
    ...
    def publish(self, CC, lac_log):
        ...
        if self.sm['selfdriveState'].active:
            ...
            if self.CP.steerControlType == car.CarParams.SteerControlType.angle:
                self.steer_limited_by_controls = abs(
                    CC.actuators.steeringAngleDeg - CO.actuatorsOutput.steeringAngleDeg) > STEER_ANGLE_SATURATION_THRESHOLD
            else:
                self.steer_limited_by_controls = abs(
                    CC.actuators.torque - CO.actuatorsOutput.torque) > 1e-2
        ...
        steer_angle_without_offset = math.radians(
            CS.steeringAngleDeg - lp.angleOffsetDeg)
        cs.curvature = - \
            self.VM.calc_curvature(
                steer_angle_without_offset, CS.vEgo, lp.roll)
        ...
        cs.forceDecel = bool((self.sm['driverMonitoringState'].awarenessStatus < 0.0) or (
